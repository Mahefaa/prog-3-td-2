...
public class PlayerMapperTest {
  ...
  void player_to_entity_ok() {
    when(teamRepositoryMock.findByName(teamBarea().getName())).thenReturn(Optional.of(teamBarea()));
  ...
  void player_to_entity_ko() {
    when(teamRepositoryMock.findByName(teamBarea().getName())).thenReturn(Optional.empty());
    assertThrowsExceptionMessage("404 NOT_FOUND : Team#Barea not found.", NotFoundException.class,
        () -> subject.toEntity(
            playerModelRakoto(playerEntityRakoto(TeamEntity.builder().build()))));
  }
}
