...
public class PlayerMapper {
  ...
  public PlayerScoreEntity toEntity(int matchId, PlayerScorer scorer) {
    PlayerEntity player = playerRepository.findById(scorer.getPlayer().getId())
        .orElseThrow(() -> new NotFoundException("Player#" + scorer.getPlayer().getId() + " not"
            + " found."));
  ...
  public PlayerEntity toEntity(Player domain) {
    TeamEntity team = teamRepository.findByName(domain.getTeamName())
        .orElseThrow(() -> new NotFoundException(
            "Team#" + domain.getTeamName() + " not found."));
